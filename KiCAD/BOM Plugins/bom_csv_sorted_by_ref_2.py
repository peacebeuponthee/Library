...
"""
    ...
    Fields are (if exist)
    Ref, value, Part, footprint, Datasheet, Vendor Part Number, Vendor
...
writerow( out, ['Ref', 'Value', 'Vendor', 'Vendor Part Number', 'Footprint', 'Datasheet'] )

# Output all of the component information (One component per row)
for c in components:
    writerow( out, [c.getRef(), c.getValue(), c.getField("Vendor"),
        c.getField("VendorID"), c.getFootprint(), c.getDatasheet()])
